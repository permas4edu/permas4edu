...
#   ax1.spy(BMLL)
    ...
    pass
# fig1 = plt.figure('Modal assurance criteria I')
# ax1  = fig1.add_subplot(111)
# ax1.set_title('MAC')
# cax  = ax1.imshow(MAC.todense(),cmap=cm,interpolation='nearest',origin='lower',alpha=1)
# cbar = fig1.colorbar(cax,ticks=[0, 0.5, 1.], orientation='vertical')
# cbar.set_ticklabels([r'$0\%$', '$50\%$', '$100\%$'])# horizontal colorbar
...
ax1.set_ylabel('Modified system')
ax1.set_xlabel('Initial system')
ax1.grid(which='minor',color='w', linestyle='-',linewidth=1)

plt.colorbar(mesh1,ax=ax1)
plt.tight_layout()
plt.savefig('mac_%.2d'%int(counter),dpi=200)

#freq_ema, freq_sim = parser(res) # extract eigenfrequencies from modal test and simulation

workbook =   xlsxwriter.Workbook(os.path.join(os.path.dirname(inp),'mac_%.2d.xlsx'%int(counter)))
...
worksheet.conditional_format(1,1,p+1,q+1,{'type': 'cell',
                                          ...
                                          'value':0.8,
                                          'format':format1})
worksheet.insert_image('D30','mac_%.2d.png'%(int(counter)) , {'x_scale': 1.0, 'y_scale': 1.0})
