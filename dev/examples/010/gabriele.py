    ...
    while 1:
        ...
        if re.search('RVALTAB.DISP',line):
            ...
            while 1:
                ...
                if ic == 1: freq_ema.append(liste[1])
                if ic == 2: freq_sim.append(liste[1])
...
plt.tight_layout()
plt.savefig(inp[:-5]+'.png',dpi=200)

workbook =   xlsxwriter.Workbook(os.path.join(os.path.dirname(inp),inp[:-5]+'.xlsx'))
...
worksheet.conditional_format(1,1,p+1,q+1,{'type': 'cell',
                                          ...
                                          'value':0.8,
                                          'format':format1})
worksheet.insert_image('D30',inp[:-5]+'.png' , {'x_scale': 1.0, 'y_scale': 1.0})
