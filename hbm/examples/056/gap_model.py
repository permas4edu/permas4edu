...
kr=[10.,100.,1000.,10000.,100000.]
for i, csv in enumerate(csvs):
    df = pd.read_csv(csv,delimiter=';')
    ax1.plot(2*np.pi*df['Frequency'],df['H1-N101,u'],lw=2,label=r'$k_y=%d$ [N/m$^3]$' %kr[i])
ax1.grid()
ax1.set_xlabel(r'Excitation Frequency $\Omega$ ')
ax1.set_xlim(0.0,20.0)
ax1.set_ylim(0.0,40.0)
ax1.set_xticks(np.linspace(0.0,20,5))
ax1.set_yticks(np.linspace(0.0,40,5))
