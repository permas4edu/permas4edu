...
fig, axs = plt.subplots(2, 2, figsize=(15, 15), num='Transient Response Valanis model')
...
for i, csv1 in enumerate(csv1s):
    ...
    if i == 0:
        ...
        axs[0,0].set_xticks(np.linspace(-0.05,0.05,3))
        axs[0,0].set_yticks(np.linspace(-0.8,0.8,9))
    ...
    if i == 1:
        ...
        axs[0,1].set_xticks(np.linspace(-0.03,0.03,7))
        axs[0,1].set_yticks(np.linspace(-0.3,0.3,7))
        axs[0,1].set_title(r'$\omega/\omega_n=1.05$')
    if i == 2:
        axs[1,0].scatter(df1['N101,u'],df1['N1001,u'],s=10,c=df1['Time'].values,cmap='coolwarm',marker='_')
        axs[1,0].set_xticks(np.linspace(-0.02,0.02,5))
        axs[1,0].set_yticks(np.linspace(-0.2,0.2,9))
        axs[1,0].set_ylim(-0.2,0.2)
        axs[1,0].set_title(r'$\omega/\omega_n=1.25$')
    if i == 3:
        axs[1,1].scatter(df1['N101,u'],df1['N1001,u'],s=10,c=df1['Time'].values,cmap='coolwarm',marker='_')
        axs[1,1].set_xticks(np.linspace(-0.2,0.2,5))
        axs[1,1].set_yticks(np.linspace(-0.6,0.6,7))
        axs[1,1].set_xlim(-0.2,0.2)
        axs[1,1].set_title(r'$\omega/\omega_n=1.4$')
for i, ax in enumerate(axs.flat):
    if i < 2:
        ax.set(xlabel='Displacement $x$', ylabel ='Velocity $\dot{x}$')
    else:
        ax.set(xlabel='Displacement $x$', ylabel ='Force $F_{nl}$')
